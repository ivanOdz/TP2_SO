...
int snake(int argc, char** argv){
    ...
    if (argc >= 2){
        ...
        if (players > 2) players = 2;    // De argumento
    ...
    while (1) {
        ...
        if (mNextFrame < SyscallGetTicks()) {
            ...
            if (lost) {
                ...
                if (flags.lose1 && flags.lose2)                       { winner = 0; }
                else if (flags.lose1 && flags.players == 2)           { winner = 2; }
                else if (flags.lose2)                                 { winner = 1; }
...
void printScore(playersFlags flags){
    ...
    if(flags.players == 2) {printf("\tPlayer 2: %5d ", snakePos2.score);}
}
...
uint8_t check_collision(playersFlags *flags, position* nextHead1, position* nextHead2) {
    ...
    do {
        ...
        if (nextHead1->x == snake1[cont].x && nextHead1->y == snake1[cont].y)        { flags->lose1 = 1; break;} // Colisiona Snake 1 con sigo misma
        else if (flags->players==2 && nextHead2->x == snake1[cont].x && nextHead2->y == snake1[cont].y)  { flags->lose2 = 1; break;} // Colisiona Snake 2 con Snake 1
    ...
    if (flags->players == 2) {
        ...
        do {
            ...
            if (nextHead2->x == snake2[cont].x && nextHead2->y == snake2[cont].y)      { flags->lose2 = 1; break;} // Colisiona Snake 2 con sigo misma
            else if (nextHead1->x == snake2[cont].x && nextHead1->y == snake2[cont].y)  { flags->lose1 = 1; break;} // Colisiona Snake 1 con Snake 2
...
void move_heads(playersFlags *flags, position *nextHead1, position *nextHead2) {
    ...
    if (flags->grow1 | flags->grow2) newApple(flags);
...
void newApple(playersFlags* flags){
    ...
    do{
        ...
        do {
            if (apple.x == snake1[cont].x && apple.y == snake1[cont].y) { collision = 1; break;} // Manzana sobre snake 1
            cont++;
            cont&=0x1FF;
        } while (cont != ((snakePos1.head + 1) & 0x1FF));
        if(flags->players == 2 && !collision){
            cont = snakePos2.tail;
            do {
                if (apple.x == snake2[cont].x && apple.y == snake2[cont].y) { collision = 1; break;} // Manzana sobre snake 2
                cont++;
                cont&=0x1FF;
            } while (cont != ((snakePos2.head + 1) & 0x1FF));
        }
...
uint8_t check_positions(playersFlags *flags, position *nextHead1, position *nextHead2) {
    ...
    if (check_limits(flags, nextHead1, nextHead2)) return 1;

    if (check_collision(flags, nextHead1, nextHead2)) return 1;

    if ( nextHead1->x == apple.x && nextHead1->y == apple.y ) { flags->grow1 = 1; }
    if ( flags->players==2 && nextHead2->x == apple.x && nextHead2->y == apple.y ) { flags->grow2 = 1 * (flags->players == 2); }
...
uint8_t check_limits(playersFlags *flags, position* nextHead1, position* nextHead2) {
    ...
    if ( nextHead1->x >= BLOCKS_X || nextHead1->y >= BLOCKS_Y ) { flags->lose1 = 1; }

    if ( flags->players == 2 && (nextHead2->x >= BLOCKS_X || nextHead2->y >= BLOCKS_Y) ) { flags->lose2 = 1; }
